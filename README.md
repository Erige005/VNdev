> [!WARNING]
> **Phần công nghệ trong tài liệu này đã lỗi thời.**
>
> Các mục nói về Tauri, React, TypeScript, PixiJS và con số "game xuất ra
> ~10MB" thuộc hướng đi cũ đã bị bỏ. Dự án hiện dùng **C# + Godot 4**, không
> có HTML ở bất kỳ đâu. Xem `CLAUDE.md` để biết stack hiện tại và lý do đổi,
> `docs/LO-TRINH.md` để biết đang làm tới đâu.
>
> Những phần vẫn còn nguyên giá trị: bối cảnh, đối tượng người dùng, mô tả
> chi tiết tính năng, định dạng dự án dạng thư mục JSON, nguyên tắc thiết kế,
> và bảng so sánh với Ren'Py (trừ dòng dung lượng).
